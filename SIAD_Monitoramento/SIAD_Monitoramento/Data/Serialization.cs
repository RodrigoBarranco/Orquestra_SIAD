...
namespace Orquestra3_SIAD.Data
{
    ...
    public class Serialization
    {
        ...
        public void serializeTree(int codTask, Data.DecisionTree tree)
        {
            ...
            FileStream stream = new FileStream(@"C:\inetpub\wwwroot\Orquestra3_SIAD" + @"\tree" + codTask,
                                                FileMode.Create,
        ...
        {
            IFormatter formatter = new BinaryFormatter();
            Stream stream = new FileStream(@"C:\inetpub\wwwroot\Orquestra3_SIAD" + @"\tree" + codTask,
                                            FileMode.Open,
                                            FileAccess.Read,
                                            FileShare.Read);
        ...
        {
            return Accord.MachineLearning.DecisionTrees.DecisionTree.Load(@"C:\inetpub\wwwroot\Orquestra3_SIAD" + @"\tree" + codTask);
        }

        public bool hasSerializedTree(int codTask)
        {
            return File.Exists(@"C:\inetpub\wwwroot\Orquestra3_SIAD" + @"\tree" + codTask);
        }
    }
}